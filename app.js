...
function render() {
  ...
  state.objects.forEach((item) => {
    ...
    weightElement.style.top = `-${
      item.size / 2 + seesawPlank.offsetHeight / 2
    }px`;
  ...
  document.getElementById(
    "left-weight-value"
  ).textContent = `${state.totalLeftWeight.toFixed(1)} kg`;
  document.getElementById(
    "right-weight-value"
  ).textContent = `${state.totalRightWeight.toFixed(1)} kg`;
  document.getElementById(
    "next-weight-value"
  ).textContent = `${state.nextWeight}.0 kg`;
  document.getElementById(
    "tilt-angle-value"
  ).textContent = `${state.angle.toFixed(1)}°`;
}
...
seesawPlank.addEventListener("click", (e) => {
  ...
  fallingWeight.addEventListener(
    "transitionend",
    () => {
      ...
      logAction(
        `${newWeight.weight}kg dropped on ${newWeight.side} side at ${newWeight.distance}px`
      );
